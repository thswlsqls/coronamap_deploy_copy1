...
def get_status():
    ...
    cured = soup.select_one(
        '#container > div.layout-inner.layout-sub > div > div.status > div.status-seoul > div.cell-group.first-cell > div.cell.cell5 > div.num.num8 > p.counter').get_text()
...
def get_patients():
    ...
    #     else:
    #for num, row in enumerate(patients_rows):
    #   informations = [info.test for info in row.select('td')]
    #   print(informations)
    ...
    for num, rows in enumerate(patients_rows):
        ...
        # print(informations)
        # print(informations[0].replace('\t','').replace('\n','').replace('\r','').replace('<tr>','').replace('<td class="tdl" colspan="5">','').replace('<p>','').replace('</p>','').replace('</td>','').replace('</tr>','').replace('<b>','/').replace('<span>','  ').replace('</b>','').replace('</span>','').replace('<span class=\'alignR\'>',''))
        pathinfo = informations[0].replace('\t', '').replace('\n', '').replace('\r', '').replace('<tr>', '').replace(
            '<td class="tdl" colspan="5">', '').replace('<p>', '').replace('</p>', '').replace('</td>', '').replace(
            '</tr>', '').replace('<b>', ' / ').replace('<span>', '  ').replace('</b>', '').replace('</span>','').replace(
            '<span class=\'alignR\'>', '')
        patients[patient_num] = {}
        patients[patient_num]["ID"] = informations[1]
        patients[patient_num]["Gender"] = '정보없음'
        patients[patient_num]["Age"] = '정보없음'
        patients[patient_num]["Region"] = informations[4]
        patients[patient_num]["Confirmed Date"] = informations[3]
        patients[patient_num]["Current Status"] = informations[5]
        patients[patient_num]["Paths"] = informations[0]
        patient_num += 1
    #print(patients)
